    ...
    <>
      <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-4 md:p-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-3">
          <div>
            <h2 className="text-xl md:text-2xl font-bold text-gray-900">Live Market Data</h2>
            <p className="text-sm md:text-base text-gray-600 mt-1">Real-time stock prices and changes</p>
          </div>
          <div className={`px-3 md:px-4 py-2 rounded-full text-xs md:text-sm font-semibold flex items-center space-x-2 ${
            ...
            <div key={stock.symbol} className="bg-gray-50 rounded-lg p-4 border border-gray-200">
              ...
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center">
                  ...
                  <div className="ml-3">
                    <div className="text-base font-bold text-gray-900">{stock.symbol}</div>
              ...
              <div className="grid grid-cols-2 gap-3 mb-3">
                <div>
                  ...
                  <div className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold border ${getChangeColor(stock.change)}`}>
                    <span className="mr-1">{getChangeIcon(stock.change)}</span>
                    {stock.change > 0 ? '+' : ''}{stock.change.toFixed(2)}
                  </div>
                </div>
              ...
              <button
                onClick={() => openChart(stock)}
                className="w-full text-blue-500 hover:text-blue-700 font-medium text-sm py-2 border border-blue-300 rounded-lg hover:bg-blue-50 transition-colors"
              >
                View Chart
              </button>
            </div>
        ...
        {stocks.length === 0 && (
          <div className="text-center py-12">
            ...
            <h3 className="text-base md:text-lg font-medium text-gray-900 mb-2">Waiting for market data</h3>
            <p className="text-sm md:text-base text-gray-500">Live stock prices will appear here once connected</p>
          </div>
        )}
